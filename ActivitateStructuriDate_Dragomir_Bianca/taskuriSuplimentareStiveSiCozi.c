...
struct NodDubluStiva {
	Masina info;
	struct NodDubluStiva* prev;
	struct NodDubluStiva* next;
};
typedef struct NodDubluStiva NodDubluStiva;

void pushStivaDublu(NodDubluStiva** top, Masina m) {
	NodDubluStiva* nou = (NodDubluStiva*)malloc(sizeof(NodDubluStiva));
	nou->info = m;
	nou->next = *top;
	nou->prev = NULL;
	if (*top) (*top)->prev = nou;
...
Masina popStivaDublu(NodDubluStiva** top) {
	Masina m;
	m.id = -1;
	if (*top) {
		NodDubluStiva* tmp = *top;
		m = tmp->info;
		*top = tmp->next;
...
//5.
struct NodCoadaSimpla {
	Masina info;
	struct NodCoadaSimpla* next;
};
typedef struct NodCoadaSimpla NodCoadaSimpla;

struct CodaSimpla {
	NodCoadaSimpla* first;
	NodCoadaSimpla* last;
};
typedef struct CodaSimpla CoadaSimpla;

void enqueueSimpla(CoadaSimpla* c, Masina m) {
	NodCoadaSimpla* nou = (NodCoadaSimpla*)malloc(sizeof(NodCoadaSimpla));
	nou->info = m;
	nou->next = NULL;
...
Masina dequeueSimpla(CoadaSimpla* c) {
	Masina m;
	m.id = -1;
	if (c->first) {
		NodCoadaSimpla* tmp = c->first;
		m = tmp->info;
		c->first = tmp->next;
